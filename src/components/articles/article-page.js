...
function ArticlesPage({ id, state, getCurrentArticle }) {
  useEffect(() => {
    if (id !== undefined) {
      getCurrentArticle(id)
    }
  }, [])
  console.log('bool', Boolean(state.article))
  let element = <h1>TEst</h1>
  if (state.article) {
    const { article } = state
    const { title, description, body, favoritesCount, author, createdAt, tagList } = article
