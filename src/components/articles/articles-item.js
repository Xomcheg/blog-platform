...
function ArticlesItem({ item, articleClick }) {
  const { title, description, favoritesCount, author, createdAt, tagList } = item
  const { username, image } = author
  const createDate = format(new Date(createdAt), 'PPP')
  const tags = tagList.map((tag) => <ArticlesItemTag tag={tag} />)
  return (
    <div className={articles.articles__item} onClick={articleClick} aria-hidden="true">
